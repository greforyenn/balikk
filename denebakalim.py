...
#     url = "https://github.com/greforyenn/rina/blob/main/SerialKeys"
...
# def fetch_hwid_id():
#     url = "https://github.com/greforyenn/rina/blob/main/Hwid.txt"
#     response = requests.get(url)
#     if response.status_code == 200:
#         return response.text.strip().split("\n")
#     else:
...
def balikvar():
    try:
        # Sadece envanter bölgesini kontrol et (örnek bölge ayarlanmalı)
        # Bu bölgeyi kendi ekran çözünürlüğüne göre düzenle
        region = (750, 100, 250, 400)  # Örnek: sol üst x, y, genişlik, yükseklik
        screenshot_location = pyautogui.locateOnScreen(balik, confidence=0.75, region=region)
        return screenshot_location is not None
    except Exception as e:
...
def detect_object(envanter_dolu_mu, is_paused):
    while True:
        if envanter_dolu_mu.value or is_paused.value:
            time.sleep(1)
            continue
            ...
            for box in boxes:
                ...
                if not telsizbuldu() or envanterdoldu == False:  
                    if cls == 0 and conf >= 0.80:
                     ...
                     print("[LOG] Emoji tespit edildi, diğer işlemler durduruldu.")
                     time.sleep(5)  # Emoji geçme süresine göre değiştirilebilir
                     is_paused.value = False
                     print("[LOG] Emoji geçti, işlemler devam ediyor.")
                    if cls == 4:
                        if conf >= 0.80:
                            cx, cy = x1 + w // 2, y1 + h // 2 + 5
                            click_to_position(cx, cy, 0)
                    if cls == 5:
                        if conf >= 0.80:
                            global farekanks
                            farekanks = True
                    if cls == 9 and conf >= 0.60:
                        is_paused.value = True
                        print("[LOG] Puzzle tespit edildi,diğer işlemler durduruluyor.")
                        global px, py
                        px, py = x1 + w // 2, y1 + h // 2
                        puzzlebot()
                        is_paused.value = False
                        print("[LOG] Puzzle tamamlandı, işlemler devam ediyor.")
                    if cls == 10:
                        if conf >= 0.60:
                            global px2, py2
...
def puzzlebot():
    # 1. Ekran görüntüsü al (Puzzle'ı çizdirmek için)
    screen = np.array(ImageGrab.grab(bbox=(0, 30, 1024, 768)))
    debug_image = screen.copy()

    # 2. Koordinatları ekrana çiz (Başlangıç: Mavi, Bitiş: Yeşil)
    cv2.circle(debug_image, (px3, py3), 10, (255, 0, 0), -1)  # Başlangıç noktası: Mavi
    cv2.circle(debug_image, (px, py3), 10, (0, 255, 0), -1)   # Hedef noktası: Yeşil

    # 3. Çizgi çiz
    cv2.line(debug_image, (px3, py3), (px, py3), (0, 255, 255), 2)

    # 4. Ekranı göster
    cv2.imshow("Puzzle Debug", debug_image)
    cv2.moveWindow("Puzzle Debug", 800, 0)
    cv2.waitKey(500)  # 500ms göster sonra otomatik kapanır
    ...
    pyautogui.mouseDown()
    time.sleep(0.5)
    pyautogui.moveTo(px+3, py3, duration=2)
    time.sleep(1)
    pyautogui.mouseUp()
        ...
        for session in sessions:
            if session.Process and session.Process.name().lower() == "ragemp_game_ui.exe".lower():
                ...
                try:
                    while True:
                        if dolduenvanterama():
                            ...
                            while balikvar():
                                ...
                                if timer is not None:
                                    timer.cancel()
                                screenshot_location = pyautogui.locateOnScreen(balik, confidence=0.75)
                                if screenshot_location is not None:
                                    ...
                                    time.sleep(0.2)
                                    try:
                                        bagajaat = pyautogui.locateOnScreen(bagaj, confidence=0.7)
                                    except pyautogui.ImageNotFoundException:
                                        bagajaat = None
                                    ...
                                    if bagajaat is not None:
                                        ...
                                        print("bagaj görseli bulunamadı")
                            print("[LOG] Envater boşaltıldı.")
                            envanter_dolu_mu.value = False
                            pyautogui.press('tab')
                            time.sleep(3)
                            pyautogui.press('f7')
                            time.sleep(3)
                    ...
                    break
        if not bulundu:
            print(f"[LOG] İşlem bulunamadı, tekrar deniyorum")
            time.sleep(1)
...
if __name__ == "__main__":
    envanter_dolu_mu = Value('b', False)
    is_paused = Value('b', False)
    ...
    tespit_thread = Process(target=sestakip, args=(envanter_dolu_mu,))
    okeylet_thread = Process(target=detect_object, args=(envanter_dolu_mu, is_paused))
    try:
